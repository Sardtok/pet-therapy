...
class Gravity(Capability):
    ...
    def do_update(self, collisions: List[Collision], time: float):
        if self.subject.state == EntityState.drag: return False
        if self._animation_requires_no_gravity(): return False
    ...
    def _ground_level(self, collisions: List[Collision]) -> float:
        ...
        touches_ground = lambda c: c.intersection.min_y == ground_level
        surface_contact = sum([c.intersection.width for c in ground_collisions if touches_ground(c)])
    ...
    def _start_falling_if_needed(self) -> bool:
        if self.is_falling: return False
    ...
    def _animation_requires_no_gravity(self) -> bool:
        ...
        if animation.anchor_point != EntityAnimationAnchorPoint.bottom: return True
