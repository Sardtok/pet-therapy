...
class BounceOnLateralCollisions(Capability):
    def do_update(self, collisions: List[Collision], time: float):
        if self.subject.is_ephemeral or self.subject.state != EntityState.move: return
    ...
    def bouncing_angle(self, current_angle: float, collisions: List[Collision]) -> float:
        ...
        if not target_side: return None
        if not _contains_overlap_on_side(valid_collisions, target_side): return None
        if _contains_overlap_on_side(valid_collisions, target_side.opposite): return None
        return math.pi - current_angle
    
    def target_side(self) -> CollisionSide:
        direction = self.subject.direction.dx
        if direction < -0.0001: return CollisionSide.left
        if direction > 0.0001: return CollisionSide.right
        return None

        
def _contains_overlap_on_side(collisions, target_side) -> bool:
    valid_collisions = filter(lambda c: not c.is_ephemeral and c.is_overlapping, collisions)
