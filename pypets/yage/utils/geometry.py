...
class Point:
    ...
    def offset(self, **kwargs):
        if kwargs.get('size'): x = kwargs['size'].width
        elif kwargs.get('point'): x = kwargs['point'].x
        else: x = kwargs.get('x') or 0
        if kwargs.get('size'): y = kwargs['size'].height
        elif kwargs.get('point'): y = kwargs['point'].y
        else: y = kwargs.get('y') or 0
...
def degrees_to_radians(degrees):
    ...
    if degrees < 0: return degrees_to_radians(degrees + 360)
    if degrees >= 360: return degrees_to_radians(degrees - 360)
